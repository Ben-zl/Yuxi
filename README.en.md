...
Yuxi is a **self-hosted, multi-tenant knowledge agent platform**. Rather than stopping at a chat interface, it brings **RAG retrieval, Milvus-backed knowledge graphs, AgentScope multi-agent execution, MCP/Skills, sandbox tools, and access control** into one workspace.
...
- 🤖 **Agent development** — Built on AgentScope, with Team workers, Skills, MCPs, tools, middleware, Session persistence, and workspace isolation; long-running tasks run asynchronously on a background worker and expose previewable, downloadable artifacts.
...
| Backend | FastAPI · AgentScope · ARQ (async worker) |
...
- [AgentScope](https://github.com/agentscope-ai/agentscope) - Provides Session, tool, Team, workspace, and model execution primitives.
