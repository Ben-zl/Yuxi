...
`reject` 用于不接受排队的调用。只要请求不能立即成为并派发 FIFO 队头，系统就记录并返回拒绝状态，不创建 Agent 运行。这包括线程忙碌、已有积压请求，以及运行正在等待人工回答的情况。
...
因此，已经开始的模型调用或工具批次可能在中断生效前完成；Steer 会请求中断当前 AgentScope session，当前 Run 以实际终态收束，随后派发的新 Run 继续复用同一 session 上下文。已有普通 Chat 排队项也可以原地提升为 Steer；同一线程一次只接受一个待处理 Steer。
...
- Steer 在等待活跃 Run 到达安全点时不能取消，避免取消操作与运行终态接力竞态。
...
Agent 运行进入 completed、failed 或 cancelled 终态后都会自动派发下一条请求；失败的 Run 本身不重试，也不能阻塞其后的独立输入。需要人工处理的中断状态按以下规则处理后续请求：

- failed/cancelled 时已经在等待的请求按 FIFO 自动接力；队列为空时，之后提交的新请求也可正常执行。
...
若终态后的自动派发因短暂故障遗漏，启动恢复会把无活跃 Run 的队头识别为 ready 并继续派发，而不会把仍有请求的队列视为已空闲。
