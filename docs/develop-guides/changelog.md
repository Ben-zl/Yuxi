...
- agentscope 承接迁移完成网关翻转（工单 14①②⑤⑥⑦执行面）：ARQ worker 执行体切换为 agentscope 管道（配置投影/协议转换/消息落库/终态回写），存量线程按切换时间戳只读拒发，steer 经会话中断提前结束，run_worker 旧 LangGraph 路径移除；前端零改动联调通过（真实浏览器 + MiniMax-M3）。真实浏览器全量验证后修复：审批 chunk 补齐 `approval` 包装、`review_configs` 等长与工具入参 JSON 解析（模态框此前不弹出）、resume 路径终态回写（Run 不再卡 running）、steer 中断后队头立即派发且 `interrupted` 仅在审批挂起时保持等待、steer 中断移至入队事务提交后消除派发竞态、worker 关机钩子移除对不存在方法的调用。第二轮浏览器验证（并发/长任务/取消/技能/MCP/知识库/Team）后修复：Run 取消信号接入网关取消监听器（中断会话、终态归一 cancelled，此前取消后仍跑完并误标 completed）；run 级 model_spec 贯通至会话投影（用户模型选择此前在无模型智能体上被丢弃）；无模型智能体回落系统默认模型（与旧栈一致）；「深度研究」提示词的子智能体派发指令从旧栈 task 工具更正为 TeamCreate/AgentCreate/TeamSay 团队工具链。第三轮沙盒/子智能体/审批深化验证后修复：resume 续跑中再次挂起审批时补发审批事件（此前弹窗不出现、180s 超时误判失败），审批挂起缓存对 resume 路径同样生效；终态竞态用例迁移至 AgentRunRepository（旧 run_worker 符号清理）。缺口修复：并行多工具审批补全——resume 确认前以会话中 asking 完整集合为准，事件流停滞期间每 15 秒自愈补发确认（修复第二个 Write 永久滞留 asking 导致的 180s 超时）；team 编排轮次增加静默收束窗口（REPLY_END 后 45 秒内成员回报驱动的续写被聚合回流，总预算 600 秒），普通轮次行为不变。此前交付：agentscope 服务、桥接层与 38+ 项测试全绿：新增 agentscope 服务（独立 database 存储、Redis bus、Docker workspace 沙盒）与 yuxi/agentscope 桥接层（统一配置投影、协议转换、线程队列执行器、审批/中断、Team 子智能体、KB/MCP/Skills 工具、OTel 观测与用量归集）；fork 固定 commit ae6a563c 经 lock 可复现构建；新增 38 项单元/集成/e2e 测试在 docker compose 真实环境全绿（含 MiniMax-M3 真实模型链路与 Langfuse OTLP span 上报实证）。
