...
- 新增 agentscope 承接迁移首块基建（工单 01）：fork 以 git 依赖固定 commit 接入 lock 并可复现构建；新增 agentscope 服务（PostgreSQL 独立 database 持久存储 + Redis message bus + 本地 workspace 骨架），与现有 api/worker 容器并存，不影响现有智能体链路。
