...
- agentscope 承接迁移完成网关翻转（工单 14①②⑤⑥⑦执行面）：ARQ worker 执行体切换为 agentscope 管道（配置投影/协议转换/消息落库/终态回写），存量线程按切换时间戳只读拒发，steer 经会话中断提前结束，run_worker 旧 LangGraph 路径移除；前端零改动联调通过（真实浏览器 + MiniMax-M3）。此前交付：agentscope 服务、桥接层与 38+ 项测试全绿：新增 agentscope 服务（独立 database 存储、Redis bus、Docker workspace 沙盒）与 yuxi/agentscope 桥接层（统一配置投影、协议转换、线程队列执行器、审批/中断、Team 子智能体、KB/MCP/Skills 工具、OTel 观测与用量归集）；fork 固定 commit ae6a563c 经 lock 可复现构建；新增 38 项单元/集成/e2e 测试在 docker compose 真实环境全绿（含 MiniMax-M3 真实模型链路与 Langfuse OTLP span 上报实证）。
