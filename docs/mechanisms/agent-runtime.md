...
本页解释 Yuxi 如何从持久化 Agent 配置构建一次运行，以及配置、权限、文件和 AgentScope Session state 在运行中分别负责什么。Agent 配置和扩展开发见[配置和开发智能体](../agents/agents-config.md)。
...
flowchart LR
    ...
    Prepare --> Projection["RuntimeProjection"]
    Projection --> Session["AgentScope Session / Execution"]
    Session --> Result["消息、事件、文件和产物"]
```
...
5. 没有配置模型时，系统读取管理员设置的默认模型；然后 `RuntimeProjection` 为 AgentScope Session 创建请求、模型、工具和中间件。
6. AgentScope state 保存 Session 的消息、待办、上下文压缩和子智能体状态；AgentScope storage 使用独立 PostgreSQL，Yuxi PostgreSQL 保存对应的业务投影。
...
| AgentScope Session state | Session 执行和 middleware | 当前 Session |
...
当前 Project 的 `workdir_path` 决定 Agent 的默认工作目录。普通 Agent 和子 Agent 共享根 Conversation 的 Workdir 与 execution runtime；子 Agent 使用 AgentScope Team/Session 管理自己的执行状态，不从相邻 Run 推断文件结果。
...
审批或用户问题中断时，系统把中断信息保存在对应 AgentScope Session 与 Yuxi Run 挂起投影。resume 会根据线程绑定的 Agent 和当前用户重新构建 RuntimeProjection，再创建新的 Run；它不会从相邻 Run 猜测模型、工具或文件结果。
...
- [Runtime projection](https://github.com/xerrors/Yuxi/blob/main/backend/package/yuxi/agentscope/config_projection.py)
- [AgentScope middleware](https://github.com/xerrors/Yuxi/blob/main/backend/package/yuxi/agentscope/middleware.py)
