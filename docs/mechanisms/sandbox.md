...
`runtime_scope_id` 是一次顶层执行树的沙盒分组键，当前使用根 Conversation 的 thread ID。根 Agent 和子 Agent 共享这个 scope，因此可以共享同一个运行时、`/tmp`、环境和文件挂载；子 Agent 的 AgentScope Session 只隔离执行状态。
...
| 运行类型 | Session/Execution state | runtime scope | Workdir |
| --- | --- | --- | --- |
| 普通 Agent | 当前 AgentScope Session | 根 thread | 当前 Project 的 Workdir |
| 子 Agent | Team child Session | 根 thread | 继承根 Conversation |
