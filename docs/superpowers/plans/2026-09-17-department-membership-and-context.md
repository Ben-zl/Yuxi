...
## 外部 Review 第二轮修复执行记录（2026-09-20）

| Review 问题 | 修复 | 证据 |
|---|---|---|
| [P1] 调度 failed 记录非幂等（DST 回拨两周期同键→唯一约束→扫描事务失败） | `_record_failed` 在任务锁内先 `find_idempotent` 复用既有事实再插入 | 新增 DST 重复周期用例：同键两轮扫描 failed 恰 1 条、next_run_at 正常推进 |
| [P1] 旧移除确认框可能删另一部门成员 | 确认框打开时捕获部门+epoch；部门切换 watch 立即 destroy；onOk 校验捕获值失配即取消 | 新增 5 用例（接线断言/销毁幂等/部门切换拒绝/epoch 失配拒绝/正常路径用打开时部门） |
| [P1] 旧请求失效处理覆盖并发切换 | `invalidate_stale_active_department` 改条件更新（仅会话仍指向读取时旧部门才命中）+返回命中标志；中间件未命中时按会话现值重解析 | 新增存储层竞态用例：并发切换已提交→失效不命中、会话保持 B；对照断言正常失效路径仍命中 |
| [P2] 删除部门确认文案承诺迁入默认部门 | 文案改为新边界（不迁移/保留其他关系/撤销凭证/有引用拒绝） | — |

回归：部门会话+非交互入口+派发单测 64P；web 262P（新增 5）+lint+build；ruff 改动文件通过；contracts 通过。
