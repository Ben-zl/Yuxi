...
async def authorize_agent_config_resources(
    ...
    *,
    ...
    for field_name, expected_type in _AGENT_MODEL_FIELDS.items():
        ...
        provider = await get_model_provider_for_user(db, info.resource_id or info.provider_id, user)
        if provider is None or not provider.is_enabled:
            raise ValueError(f"Agent {field_name} 引用的模型资源不可访问")
        if not await _resource_covers_agent_scope(
            db,
        ...
        context[field_name] = canonical_spec
...
async def _resource_covers_agent_scope(
    db: AsyncSession,
    *,
    ...
    """确保依赖资源对 Agent 的全部可访问用户保持可读。"""
    resource_config = normalize_permission_config(resource_share_config)
    agent_config = normalize_permission_config(agent_share_config)
    target_scopes = [scope for scope in (agent_config["read_scope"], agent_config["manage_scope"]) if scope]
