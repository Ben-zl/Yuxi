...
**生成报告后，必须：**
1. 使用 **write** 工具将报告保存到本地
2. 文件名格式：`TDR_performance_trend_{天数}_{YYYYMMDD_HHMMSS}.md`
   - 例如：`TDR_performance_trend_3days_20260317_013500.md`
3. 使用 **message** 工具将报告文件发送到群聊
...
└─ 是
   ...
   └─ 用户是否明确指定分析单个案例？
      ...
      │
      └─ 否（默认）→ 执行 perfeye 多任务性能对比
                       └─ 从缓存文件中获取该设备对应案例的【最早】和【最新】两份 UUID
                           └─ 使用两个 UUID 进行多任务性能对比分析
```
...
**默认模式（多任务性能对比）**：
```
使用 perfeye-analysis skill 对以下数据进行多任务性能对比分析：
