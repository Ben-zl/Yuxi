...
async def resolve_runtime_projection(
    ...
    *,
    ...
    run = await AgentRunRepository(db).get_active_run_by_thread_for_user(
        uid=user_id, agent_slug=mapping.agent_slug, conversation_thread_id=mapping.thread_id,
    )
    ...
    return await load_run_resources(
        db, uid=user_id, manifest=run.manifest, agent_slug=runtime_agent_slug or mapping.agent_slug,
    )
