...
DEFAULT_WORKER_ID = os.getenv("YUXI_WORKER_ID", f"process-{os.getpid()}")
...
class AgentRunRepository:
    ...
    async def set_output_message(
        ...
        *,
        ...
        owner = worker_id or run.worker_id
        if owner is not None:
            self._require_lease_owner(run, worker_id=owner, now=current_time, action="持久化输出消息")
    ...
    async def set_terminal_status(
        ...
        *,
        ...
        attempt_owner = run.worker_id
        if run.status == "pending":
            if worker_id is not None or status not in {"failed", "cancelled"}:
                return run, False
        elif run.status in LEASED_RUN_STATUSES:
            owner = worker_id or run.worker_id
            if owner != run.worker_id or run.lease_expires_at is None or run.lease_expires_at <= current_time:
                return run, False
        ...
        if status == "completed" and run.worker_id is not None:
        ...
        await self._finish_open_attempt(
            run.id,
            worker_id=worker_id or attempt_owner,
