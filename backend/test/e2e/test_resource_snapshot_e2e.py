...
async def test_resource_snapshot_survives_permission_and_credential_change(e2e_client, e2e_headers):
    ...
    try:
        ...
        response = await client.post("/api/departments", headers=root, json={
            "name": f"snapshot-{suffix}", "admin_uid": login_name, "admin_password": password,
        })
        ...
        share = {"version": 2, "read_scope": {
            "access_level": "department", "department_ids": [department_id], "user_uids": [],
        }, "manage_scope": None}
        response = await client.post("/api/system/model-providers", headers=root, json={
            "provider_id": f"snapshot-{suffix}", "display_name": "Snapshot E2E",
            "base_url": "http://openai-mock:8080/v1", "api_key": "snapshot-fixture-key",
            "capabilities": ["chat"], "enabled_models": [{"id": "snapshot-proof", "type": "chat"}],
            "share_config": share,
        })
        ...
        response = await client.post("/api/agent", headers=user_headers, json={
            "slug": agent_slug, "name": "Snapshot E2E", "backend_id": "ChatbotAgent", "share_config": share,
            "config_json": {"context": {"model": spec, "system_prompt": "Reply briefly.",
                "tools": [], "mcps": [], "skills": [], "knowledges": [], "subagents": []}},
        })
        assert response.status_code == 200, response.text
        response = await client.post("/api/chat/thread", headers=user_headers, json={
            "agent_id": agent_slug, "title": f"snapshot-{suffix}",
        })
        assert response.status_code == 200, response.text
        thread_id = response.json().get("thread_id") or response.json()["id"]
        response = await client.post("/api/agent/runs", headers=user_headers, json={
            "agent_slug": agent_slug, "thread_id": thread_id, "query": "snapshot proof",
        ...
        response = await client.put(f"/api/system/model-providers/{provider_id}", headers=root, json={
            "api_key": "replacement-key", "base_url": "http://127.0.0.1:1/v1",
            "share_config": {"version": 2, "read_scope": {
                "access_level": "department", "department_ids": [1], "user_uids": [],
            }, "manage_scope": None},
        })
        assert response.status_code == 200, response.text
        rejected_request_id = f"snapshot-rejected-{suffix}"
        rejected = await client.post("/api/agent/runs", headers=user_headers, json={
            "agent_slug": agent_slug, "thread_id": thread_id, "query": "must reject",
            "meta": {"request_id": rejected_request_id},
        })
        assert rejected.status_code == 422, rejected.text
        assert rejected.json() == {"detail": f"模型资源不可访问: '{spec}'"}
        assert await db.fetchval(
            "SELECT count(*) FROM agent_run_requests WHERE request_id=$1", rejected_request_id
        ) == 0
