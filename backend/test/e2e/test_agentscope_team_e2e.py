...
真实链路：管理员配置的子智能体（启动期快照为 worker 模板）→ 主智能体
TeamCreate 组队 → AgentCreate 以自定义模板创建 worker → worker 以模板
系统提示独立会话执行 → 结果回流 leader 汇总。前置：夹具子智能体行在
服务启动前存在（验证脚本负责种子与重启）。
"""
...
async def test_team_choreography_with_custom_template(db_session):
    ...
    # leader 事件流含组队与建员工调用
    called = {
        ev.get("tool_call_name")
        for ev in result.events
        if str(ev.get("type", "")).upper() == "TOOL_CALL_START"
    }
    ...
    try:
        worker_agents = await conn.fetch(
            "SELECT payload FROM agents WHERE source = 'team'"
        )
        assert worker_agents, "应有 source=team 的 worker agent 记录"
        prompts = [
            json.loads(row["payload"])["data"]["system_prompt"]
            for row in worker_agents
        ]
        assert any("e2e 团队子智能体模板提示" in p for p in prompts), prompts

        worker_sessions = await conn.fetchval(
            "SELECT count(*) FROM sessions s JOIN agents a ON s.agent_id = a.id"
            " WHERE a.source = 'team'"
        )
