    ...
    try:
        async with async_sessionmaker(engine, expire_on_commit=False)() as db:
            ...
            share = {"version": 2, "read_scope": {
                "access_level": "department", "department_ids": [department.id], "user_uids": [],
            }, "manage_scope": None}
            provider = ModelProvider(
                resource_id=str(uuid4()), provider_id=f"snapshot-{suffix}", display_name="Snapshot",
                base_url="https://old.example.test/v1", api_key="snapshot-original-key", provider_type="openai",
                enabled_models=[{"id": "chat", "type": "chat"}], capabilities=["chat"],
                is_enabled=True, share_config=share,
            )
            mcp = MCPServer(
                resource_id=str(uuid4()), slug=f"snapshot-{suffix}", name="Snapshot", transport="streamable_http",
                url="https://old.example.test/mcp", headers={"Authorization": "snapshot-original-header"},
                enabled=1, created_by="system", updated_by="system", share_config=share,
            )
            db.add_all([provider, mcp])
            await db.flush()
            agent = Agent(
                slug=f"snapshot-{suffix}", name="Snapshot", backend_id="ChatbotAgent", created_by=uid,
                share_config=share, config_json={"context": {"model": f"{provider.resource_id}:chat",
                    "skills": [], "mcps": [mcp.resource_id], "tools": [], "subagents": []}},
            )
            db.add(agent)
            await db.flush()
            original_secret = os.environ["API_KEY_DERIVATION_SECRET"]
            reference = await capture_run_resources(
                db, uid=uid, agent_slug=agent.slug, model_spec=f"{provider.resource_id}:chat", thread_id=None,
            )
            ...
            provider.share_config = {"version": 2, "read_scope": {
                "access_level": "department", "department_ids": [other_department.id], "user_uids": [],
            }, "manage_scope": None}
            ...
            with pytest.raises(ValueError, match="身份"):
                await load_run_resources(db, uid=uid, manifest={"runtime_snapshot": {
                    **reference, "fingerprint": "tampered",
