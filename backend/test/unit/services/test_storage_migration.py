...
@pytest.mark.asyncio
@pytest.mark.parametrize("business_version", [1, 3, 4])
async def test_supported_business_schema_is_converged_and_versioned_as_current(monkeypatch, business_version):
    calls: list[str] = []
    sessions = [_Session(), _Session(), _Session()]
    ...
    assert {"create_business", "knowledge_schema"}.isdisjoint(calls)
...
async def test_failed_business_migration_does_not_record_version(monkeypatch):
    ...
    async def fail_business_schema():
        calls.append("business_schema")
        raise RuntimeError("broken business schema migration")
    ...
    manager = SimpleNamespace(
        ...
        create_business_tables=lambda: _record(calls, "create_business"),
        create_knowledge_tables=lambda: _record(calls, "create_knowledge"),
        ensure_business_schema=fail_business_schema,
        ensure_knowledge_schema=lambda: _record(calls, "knowledge_schema"),
        get_async_session_context=session_context,
        close=lambda: _record(calls, "close"),
    )
    ...
    with pytest.raises(RuntimeError, match="broken business schema migration"):
        await storage_migration.main()

    assert calls.count("business_schema") == 1
