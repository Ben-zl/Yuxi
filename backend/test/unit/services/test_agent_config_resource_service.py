...
def resources(monkeypatch):
    """提供可控资源与真实策略所查询的用户。"""
    provider = SimpleNamespace(is_enabled=True, share_config=sharing(scope("department", [1])))
    ...
    monkeypatch.setattr(
        service.model_cache,
        "get_model_info",
        lambda spec: SimpleNamespace(resource_id="resource", provider_id="legacy", model_type="chat"),
    )
    users = {
    ...
    monkeypatch.setattr(service.UserRepository, "list_by_uids", list_users)
