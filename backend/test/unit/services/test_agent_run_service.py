...
async def test_validate_agent_run_input_modalities_reads_database_when_cache_is_old(
    ...
    async def fake_get_provider(db, resource_id):
        assert db == "db"
        assert resource_id == "provider-resource"
        return SimpleNamespace(
            enabled_models=[{"id": "glm-5", "input_modalities": ["text"]}],
        )

    monkeypatch.setattr(agent_run_service, "get_model_provider_by_resource_id", fake_get_provider)
