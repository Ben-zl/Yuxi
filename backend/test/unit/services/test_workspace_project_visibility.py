...
async def test_workspace_search_hides_unbound_project_files(monkeypatch):
    class _Backend:
        def search_authorized_tree(self, *_args, **_kwargs):
            return [
                {"path": "/projects/anonymous/secret.md", "name": "secret.md", "is_dir": False, "size": 1},
                {"path": "/projects/client/report.md", "name": "report.md", "is_dir": False, "size": 2},
            ]

    monkeypatch.setattr(svc, "_workspace_backend", lambda _user: _Backend())
    monkeypatch.setattr(svc, "ProjectRepository", _ProjectRepository)

    result = await svc.search_workspace_files(
        query="report",
        current_user=SimpleNamespace(uid="user-1"),
        db=SimpleNamespace(),
    )

    assert [entry["path"] for entry in result["entries"]] == ["/projects/client/report.md"]
