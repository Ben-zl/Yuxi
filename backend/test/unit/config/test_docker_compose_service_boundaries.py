...
def test_runtime_schema_guard_detects_reintroduced_agentscope_ddl_owner() -> None:
    """负控证明 AgentScope 入口重新执行 DDL 时会被拒绝。"""
    source = "await pg_manager.ensure_business_schema()"
    ddl_markers = (
        "create_business_tables(",
        "create_knowledge_tables(",
        "ensure_business_schema(",
    ...
    assert any(marker in source for marker in ddl_markers)
