...
- 服务集成测试（`test_agentscope_workspace_integration.py`）：mock 模型发起 Write 工具调用 → 权限放行（accept_edits）→ **Write 在 Docker 沙盒容器内真实执行** → 文件经 `GET /workspace/files` 读回内容一致；产品入口的 run/viewer/附件完整链路由独立 E2E 覆盖。
