...
- [ ] 所有触发方式使用 execution_id 作为标准 Run 提交的稳定 request_id，并为线程生成确定身份。
- [ ] TaskExecution 到 AgentRun、线程的关联具有唯一性，重试不能创建第二个 Run 或线程。
- [ ] 恢复扫描先按稳定身份查找并重新关联已有 Request、AgentRun 和线程；仅在这些事实均不存在时重试派发。
- [ ] 已提交但未成功投递的 pending AgentRun 复用现有确定性 ARQ 恢复机制重新投递。
- [ ] Agent 业务失败不自动重试；只有派发基础设施中断进入幂等恢复路径。
- [ ] 故障注入集成测试覆盖提交后中断、关联前中断、投递失败和恢复进程重复运行，并证明不会重复执行。
